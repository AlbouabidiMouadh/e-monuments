...
export default function OneStreamScreen({
 
}) {
  const  {setSHowListOfStreams,setLocalParticipantMode,localParticipantId}=useContext(RoomContext)
  ...
  return (
    <KeyboardAvoidingView
      ...
      {hlsState == 'HLS_PLAYABLE'||hlsState == 'HLS_STARTED' ? (
        ...
        rooms?<WaitingScreen navigation={navigation} />:null
      ) */}
    </KeyboardAvoidingView>
  );
}
